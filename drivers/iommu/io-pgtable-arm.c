			...
			phys_addr_t paddr, size_t size, int iommu_prot)
{
	...
	struct arm_smmu_cfg hypsec_cfg = smmu_domain->cfg;
	...
	for (i = 0; i < (size / PAGE_SIZE); i++) {	
		el2_arm_lpae_map(iova, paddr, prot, hypsec_cfg.cbndx, smmu_num);
		iova += PAGE_SIZE;
		paddr += PAGE_SIZE;
	}
...
static void arm_lpae_free_pgtable(struct io_pgtable *iop)
{
	...
	size_t tlbsz = ARM_LPAE_GRANULE(data);
	__arm_lpae_free_pages(data->pgd, tlbsz, &data->iop.cfg);
#endif
	kfree(data);
}
